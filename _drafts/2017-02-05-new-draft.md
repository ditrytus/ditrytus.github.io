...
There are few additonal lines at the end of `void SpawnAsteroid()`:
...
Each asteroid prefab, comes with a `DestroyOnCollision` component. To properly explain we first have to know what it does.
...
So there is a bunch of specialized controllers and a sort of a master controller that wires up all game events. It assigna a certain amount of points for destroying an asteroid and ends the game when the player got destroyed, but without a details of these two operations. In this arrangement we have all main game logic events wired up on a high level in one place.
